...
export function computeLayout(root) {
    ...
    function traverse(node, depth, left, right) {
        ...
        const y = depth * 130 + 80

        nodes.push({ ...node, x, y })

        if (node.left) {
            const childX = (left + (left + right) / 2) / 2
            const childY = (depth + 1) * 130 + 80
            edges.push({ x1: x, y1: y, x2: childX, y2: childY, id: node.left.id, parentId: node.id })
        ...
        if (node.right) {
            ...
            const childY = (depth + 1) * 130 + 80
            edges.push({ x1: x, y1: y, x2: childX, y2: childY, id: node.right.id, parentId: node.id })
